...
/// <summary>
/// Renders the lab and imaging order sub-pages inside the Orders chart
/// tab. Shares the order sub-nav (Medications / Labs / Imaging) with the
/// medication order page so the user navigates without losing context.
/// </summary>
internal static class ServiceRequestRenderer
{
    public static string Render(ServiceRequestView view, string navBar, IReadOnlyList<ChartTab> chartTabs)
    {
        ArgumentNullException.ThrowIfNull(view);
        var sb = new StringBuilder();
        sb.Append("<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">");
        sb.Append("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">");
        sb.Append("<title>").Append(WebEncode(view.HeadingLabel)).Append(" — ").Append(WebEncode(view.PatientDisplayName)).Append("</title>");
        sb.Append(InlineCss());
        sb.Append("</head><body>");
    ...
    private static void RenderHeader(StringBuilder sb, ServiceRequestView view, IReadOnlyList<ChartTab> chartTabs)
    {
    ...
    private static string InlineCss() => @"<style>
        ...
        * { box-sizing: border-box; }
        ...
        .orders-main { max-width:1280px; margin:1.5rem auto 3rem; padding:0 1.5rem; }
